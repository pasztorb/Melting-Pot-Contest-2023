...
OBSERVATION = immutabledict.immutabledict(
    {
        ...
        "ORIENTATION": dm_env.specs.Array(shape=(), dtype=np.int32, name="ORIENTATION"),
    }
)
_ACTION = dm_env.specs.DiscreteArray(num_values=1, dtype=np.int64, name="action")
...
def rgb(height: int, width: int, name: Optional[str] = "RGB") -> dm_env.specs.Array:
...
def world_rgb(
    ascii_map: str, sprite_size: int, name: Optional[str] = "WORLD.RGB"
) -> dm_env.specs.Array:
...
def inventory(
    num_resources: int, name: Optional[str] = "INVENTORY"
) -> dm_env.specs.Array:
...
def interaction_inventories(
    num_resources: int, name: Optional[str] = "INTERACTION_INVENTORIES"
) -> dm_env.specs.Array:
...
def timestep(observation_spec: Mapping[str, dm_env.specs.Array]) -> dm_env.TimeStep:
